...
class Ghost(object):
    ...
    def __init__(
		...
		#here I think the stdout is better for log checking
    ...
    def call(self, selector, method):
        ...
        return element.evaluateJavaScript('this.%s();' % method)
	
    def get_text(self, selector):
        """Getting innerHTML in element matching given selector.

        :param selector: A CSS selector to the target element.
        """
        self.logger.debug('Getting innerHTML in `%s`' % (selector))
        element = self.main_frame.findFirstElement(selector)
        return element.toPlainText()
        
    def get_xml(self, selector):
        """Getting innerHTML in element matching given selector.
    ...
    def get_attribute(self, selector, attribute, sub_selector=""):
        """Getting attribute in element matching given selector.

        :param selector: A CSS selector to the target element.
        :param attribute: The attribute want to read.
        :param sub_selector: The child selector for the more search
        """
        self.logger.debug('Getting `%s` attribute in `%s`' % (attribute, selector))
        element = self.main_frame.findFirstElement(selector)
        if sub_selector != "" :
        	element = element.findFirst(sub_selector)
        return element.attribute(attribute, 'None')
    ...
    def set_field_value(self, selector, value, blur=True):
        ...
        def _set_text_value(el, value):
            ...
            el.evaluateJavaScript('this.value="' + value + '";')
    ...
    def wait_for(self, condition, timeout_message, timeout=None):
        """Waits until condition is True. but I don't want to raise a error when timeout
        What I want just return True if the condition is OK, and return False if the condition is met	
    ...
    def wait_for_selector(self, selector, timeout=None):
        ...
        return value, self._release_last_resources()
    ...
    def _request_ended(self, reply):
        ...
        if reply.attribute(QNetworkRequest.HttpStatusCodeAttribute):
            self.logger.info("[%s] bytesAvailable()= %s" % (
                str(reply.url().toString()),
